...
2. **Create the index shell FIRST.** If `context/index.md` doesn't exist, write it with just a `# Context index` title and an IMPORTANT directive line (e.g. `IMPORTANT: Before any task, scan the entries below. If any "Use when" matches what you're about to do, read that file first.`) and no entries. `context-index scaffold` adds entries INTO an existing index and refuses without one.
3. **Extract + index.** Move each domain section into `context/<topic>.md` with `context-index scaffold <topic> "<Use-when trigger>"`, then fill the generated body. Each file: self-contained, single-purpose, with a clear "when would I need this?" answer. If there isn't one, it shouldn't be its own file. See `references/topic-example.md` for the canonical shape. Fold topic-specific keywords (frameworks, error codes, file patterns) into each trigger - that's where they fire. Don't add a separate topic-summary line; the filename already carries the topic.
